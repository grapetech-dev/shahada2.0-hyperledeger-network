...
## Bring up the network if it is not already up.
  ...
  # configtx.yaml is mounted in the cli container, which allows us to use it to
  # create the channel artifacts
 scripts/createChannelSNM.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
    fatalln "Create channel failed"
  fi
...
function deployIdentity() {
  ...
  if [ $? -ne 0 ]; then
    fatalln "Deploying Identity chaincode failed"
  fi
...
if [[ $# -ge 1 ]] ; then
  ...
  if [[ "$key" == "createChannelSNM" ]]; then
      export MODE="createChannelSNM"
      shift
  fi
fi
...
elif [ "$MODE" == "createChannelSNM" ]; then
...
elif [ "$MODE" == "deployIdentity" ]; then
  infoln "deploying chaincode Identity on channel '${CHANNEL_NAME}'"
else
  printHelp
  exit 1
fi
